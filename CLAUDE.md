...
- `Sources/ClaudeTmpMonitor/MonitorService.swift` - Core business logic: models (`MonitoredFile`, `ClaudeProject`, `MonitorStatus`, `SizeTrend`), FSEvents + timer-based scanning, growth rate tracking, active session detection, menubar trend indicator, symlink scope/deduplication visibility, settings persistence, notifications, deletion. Exposes `onScanComplete` callback invoked at the end of each `scan()`.
