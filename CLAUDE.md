...
- `Sources/ClaudeTmpMonitor/ContentView.swift` - Main popover view: header, status bar, search/filter field with sort menu (Size/Name/Date), expandable projects list with symlink scope/deduplication badges and broken symlink counts, update banner, footer with Settings button, Stats button, Clean Broken, Clean All, and Quit. Search filters projects by display name (case-insensitive); sort order applies to both projects and files within expanded projects. Receives `onOpenSettings` and `onOpenStats` closures from `AppDelegate`. Defines `HoverButtonStyle` (custom `ButtonStyle` with configurable hover color) used by icon-only buttons.
