...
- **Settings window sizing**: fixed comically tall settings window caused by unconstrained `Spacer()` in `NSHostingController` — added fixed frame to `TabView` content and removed `.resizable` style mask from the settings `NSWindow`
